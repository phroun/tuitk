...
; can spend its plain arrows on the edit-target column. Both meanings of each
; push are bound here; a tree declares only the one its own direction answers
; to, which is what settles which of them the key reaches.
S-Left = window_size_fine_left
S-Left = trinket_sel_left
S-Left = trinket_collapse_left_or_enclosing
S-Left = trinket_expand_left_or_descend
S-Left = trinket_item_left
S-Right = window_size_fine_right
S-Right = trinket_sel_right
S-Right = trinket_collapse_right_or_enclosing
S-Right = trinket_expand_right_or_descend
